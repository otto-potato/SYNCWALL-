...
function stableDeviceCode(deviceId: number) {
  let index = Math.max(0, Math.round(deviceId) - 1);
  let width = 2;
...
async function readSyncState(env: Env) {
  const row = await env.DB.prepare(
    "SELECT command, version, target_at, video_url, media_time, timecode_rate, progress, status_message, updated_at FROM sync_state WHERE id = 1",
  ).first<{
    command: string;
    version: number;
...
async function readCalibrationState(env: Env) {
  const row = await env.DB.prepare(
    "SELECT mode, version, target_at, command_sent_at, updated_at FROM calibration_state WHERE id = 1",
  ).first<{
    mode: string;
    version: number;
    target_at: number | null;
...
const worker = {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/api/devices/heartbeat" && request.method === "POST") {
      ...
      return Response.json({
        id: device?.id,
        code: stableDeviceCode(device?.id ?? 1),
      ...
      if (request.method === "GET") {
        ...
        return Response.json({
          ...
          devices: result.results.map(
            ...
            id: device.id,
            code: stableDeviceCode(device.id),
            number: index + 1,
    ...
    if (url.pathname === "/api/calibration") {
      ...
      if (command === "auto") {
        if (current.targetAt === null) {
          return Response.json(
            { error: "freeze time before automatic calibration" },
            { status: 409 },
          );
        }
        const onlineSince = Date.now() - 3000;
        const reports = await env.DB.prepare(`
          SELECT id, clock_adjustment_ms, displayed_clock_at
          FROM devices
          WHERE last_seen >= ? AND calibration_report_version = ?
            AND displayed_clock_at IS NOT NULL
        `)
          .bind(onlineSince, current.version)
          .all<{
            id: number;
            clock_adjustment_ms: number;
            displayed_clock_at: number;
          }>();
        const statements = reports.results.map((device: {
          id: number;
          clock_adjustment_ms: number;
          displayed_clock_at: number;
        }) => {
          const correction = current.targetAt! - device.displayed_clock_at;
          const nextAdjustment = Math.max(
            -60000,
            Math.min(60000, device.clock_adjustment_ms + correction),
          );
          return env.DB.prepare(
            "UPDATE devices SET clock_adjustment_ms = ? WHERE id = ?",
          ).bind(nextAdjustment, device.id);
        });
        if (statements.length) await env.DB.batch(statements);
        return Response.json({
          ok: true,
          adjusted: statements.length,
      ...
      await env.DB.prepare(`
        ...
        SET mode = ?, version = version + 1, target_at = ?,
            command_sent_at = ?, updated_at = ?
        WHERE id = 1
      `)
        .bind(mode, targetAt, now, now)
        .run();
      return Response.json({
        ok: true,
        serverTime: Date.now(),
